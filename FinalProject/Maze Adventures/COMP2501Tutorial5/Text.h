...
public:
	...
	void render(Matrix4 &parent, std::string message)
	{
		...
		for (int i = 0; i < message.length(); i++){
			translation = translationMatrix(position + Vector3((i/* - nlIndex*/)*letterWidth, 0/*newlineCount*-(letterHeight + lineSpacing)*/, 0));
			
			mvpMatrix = parent * translation * scale;
			letterPosition = message[i] * IMG_RATIO;

			/*if (message[i] == 10 || ((i - nlIndex) - (wrapLength - 1) == 0)){
				newlineCount++;
				nlIndex = i + 1;
			}*/
